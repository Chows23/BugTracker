...
namespace BugTrackerTests
{
    ...
    public class UnitTest_TicketService
    {
        ...
        [TestMethod]
        public void GetNLatestUpdated_Should_Return_N_Latest_Updated_Tickets_Of_User()
        {
            List<Ticket> tickets = ticketService.GetNLatestUpdated(1, user);
            Assert.IsTrue(tickets.Count == 1);
            Assert.IsTrue(tickets.Exists(t => t.Id == 3));

            tickets = ticketService.GetNLatestUpdated(2, user);
            Assert.IsTrue(tickets.Count == 2);
            Assert.IsTrue(tickets.Exists(t => t.Id == 1));
            Assert.IsTrue(tickets.Exists(t => t.Id == 3));
        }
